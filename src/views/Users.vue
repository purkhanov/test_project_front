    ...
    <ConfirmPopup></ConfirmPopup>
		...
		>
					...
					<template #editor="{ data, field }">
						<Select v-model="data[field]" :options="statuses" optionLabel="label" optionValue="value" placeholder="Select a Status" fluid>
							<template #option="slotProps">
								<Tag :value="getRole(slotProps.option.value)" :severity="getStatusLabel(slotProps.option.value)" />
							</template>
						</Select>
					</template>
			...
			<Column field="is_active" header="Active" style="width: 10%">
					<template #editor="{ data, field }">
						<Select v-model="data[field]" :options="activeStatuses" optionLabel="label" optionValue="value" placeholder="Select a Status" fluid>
							<template #option="slotProps">
								<Tag :value="getActive(slotProps.option.value)" :severity="getStatusLabel(slotProps.option.value)" />
							</template>
						</Select>
					</template>
					<template #body="slotProps">
...
async function onRowEditSave(event, data) {
  ...
  try {
    const res = await axiosInstance.put(`/admin/users/${newData.id}`, {...newData})
...
function deleteConfirm(event, data) {
  confirm.require({
    ...
    accept: async () => {
      ...
      try {
        const res = await axiosInstance.delete(`/admin/users/${data.id}`)
        if (res.status === 204) {
