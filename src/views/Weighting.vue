  ...
  <div class="card flex justify-center" style="margin-bottom: 1rem;">
    ...
    <Dialog v-model:visible="visible" header="Add weight" :style="{ width: '25rem' }">
      <div class="flex items-center gap-4 mb-4" style="margin-bottom: 1rem;">
        ...
        <Select v-model="newWeighting.animal_id" size="large" :options="addWeightingSelect()" optionLabel="name" optionValue="value" placeholder="Select animal" style="width: 100%;" />
      </div>
...
async function onMountedFunc() {
  try {
    const res = await axiosInstance.get('/weightings')
...
async function addWeight() {
  ...
  try {
    const res = await axiosInstance.post('/weightings', {...payload})
    if (res.status === 201) {
      onMountedFunc()
      visible.value = false
      newWeighting.value = {}
...
async function onRowEditSave(event, data) {
  ...
  const payload = {
    animal_id: newData.nickname,
    weight_date: newData.weight_date,
    weight: newData.weight
  }

  try {
    const res = await axiosInstance.put(`/weightings/${newData.id}`, {...payload})
    if (res.status >= 200 && res.status < 300) {
      onMountedFunc()
    }
  } catch(error) {
...
function deleteConfirm(event, data) {
  confirm.require({
    ...
    accept: async () => {
      ...
      try {
        const res = await axiosInstance.delete(`/weightings/${data.id}`)
        if (res.status === 204) {
